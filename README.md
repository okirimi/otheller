...
> [!CAUTION]
> 状態ファイルの復号に失敗すると、ログにエラーが出るだけでUIは通常通り動作するため、**読み込みに失敗したことに気付かず、初期状態で開始されてしまう可能性があります**。
> 特に、同じファイルを複数回アップロードした場合、過去の暗号鍵と一致せずに整合性が失われることがあります。
> 意図しない状態で動作するリスクを避けるため、**アップロード前に `uploads` ディレクトリを一度クリアすることを推奨します**。
